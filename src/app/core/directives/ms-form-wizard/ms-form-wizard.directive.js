(function () {
  ...
  /** @ngInject */
  function MsFormWizardController($scope) {
    var vm = this;
    ...
    $scope.$watch('msWizard.selectedIndex', function (newVal, oldVal) {
      if (newVal > oldVal) {
        $scope.$emit('msWizard.stepForward', {newVal, oldVal});
      }
      if (newVal < oldVal) {
        $scope.$emit('msWizard.stepBack', {newVal, oldVal});
      }
    });
