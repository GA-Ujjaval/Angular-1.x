(function () {
  ...
  angular
    .module('app.objects')
    .controller('importbomhController', importbomhController)
  ...
  /** @ngInject */
  function importbomhController($scope, $timeout, $rootScope, $state, $http, $stateParams, hostUrlDevelopment, CustomerService, errors, $mdToast,
                                AuthService, fuseUtils, $document, introService, $mdDialog, GlobalSettingsService) {
    ...
    if (id === 'bomh') {
      vm.type = 'bomH';
      vm.dataMapping = 'bomH';
      vm.importName = 'Hierarchical BOM';
      vm.importHeading = 'Hierarchical Bill of Material';
      vm.partproductbom = 'Part';
      vm.showhideObjectnumber = true;
      vm.leveldefination = true;
      vm.showSeeMore = false;
    ...
    vm.changeUpdateBOM = changeUpdateBOM;
    isConfigurationEnable();

    function changeUpdateBOM() {
      vm.ignoreBOM = !!vm.ignoreBOM;
    }
    ...
    function changeMapping() {
      ...
      const mapping = vm.DataMapping.filter(object => object.ObjectId === vm.mapping)[0];
      vm.ignoreBOM = mapping ? !mapping.ignoreBOM : false;
    }
    ...
    function checkingMapping() {
      ...
      CustomerService.addNewMember('GET', hostUrlDevelopment.test.checkduplicatemappingH, {mappingType: vm.type,
        configName: vm.configureName})
        .then(function (response) {
          if (response.code === 0) {
            vm.data3 = vm.summeryData.newRecords === 0 ? '' : 'ab';
          } else if (response.code === 4266) {
            vm.data3 = '';
            vm.progress = false;
            $mdToast.show($mdToast.simple().textContent(response.message).action('x').toastClass('md-error-toast-theme').position('top right').hideDelay(0));
          } else {
    ...
    function fileSuccess(file, message) {
      ...
      if (ext == 'csv' || ext == 'xlsx' || ext == 'xls') {
        ...
        vm.progress = false;
        vm.dataHidden = '';
        $mdToast.show($mdToast.simple().textContent('Only CSV or XLSX or XLS file is accepted').action('x').toastClass("md-error-toast-theme").position('top right').hideDelay(0));
      }
    }
    ...
    function next() {
      ...
      switch (vm.steper) {
        ...
        case 2:
          ...
          //const foundedMapping = vm.DataMapping.filter(object => object.ObjectId === vm.mapping)[0];
          if (mapping === '') {
            //For Progress Loader
            vm.progress = true;
            ...
            if (vm.MappingData === undefined || vm.MappingData === 'No mapping configuration available.') {
              vm.MappingData = '';
            }
            vm.progress = true;
            ...
            vm.progress = true;
            CustomerService.addNewMember('POST', hostUrlDevelopment.test.addbomH, params, data, header)
              .then(function (response) {
                switch (response.code) {
                  case 0:
                    vm.steper = 3;
                    ...
                    if (vm.summeryData.importFlag === false || vm.summeryData.newRecords === 0) {
                      var failed = true;
                      angular.forEach(vm.summeryData.StatisTics, function (value, key) {
                        if (failed) {
                          if (value.importStatus === 'Failed') {
                            vm.data3 = '';
                            failed = false;
                            $mdToast.show($mdToast.simple().textContent(value.importMessage).action('x').toastClass("md-error-toast-theme").position('top right').hideDelay(0));
                          }
                        }
                      })
                    } else {
    ...
    function DoneFunction(id, type) {
      ...
      $mdToast.show($mdToast.simple().textContent('Import Hierarchical Bill of Material Parts operation was successfully completed...').position('top right'));
    }
    ...
    function autoMapped() {
      ...
      CustomerService.addNewMember('POST', hostUrlDevelopment.test.importbomH, params, '', header)
        .then(function (response) {
